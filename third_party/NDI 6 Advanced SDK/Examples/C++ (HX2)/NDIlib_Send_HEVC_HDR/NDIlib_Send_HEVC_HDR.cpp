...
namespace highQ {
#include "hevc_hdr_highQ.h"
}

// Include the preview resolution test data
namespace lowQ {
#include "hevc_hdr_lowQ.h"
}
...
void init_video_data(video_send_data* p_send_data, int num_frames, const stream_info_type& stream_info, NDIlib_FourCC_video_type_ex_e stream_type)
{
	for (int i = 0; i != num_frames; i++) {
		...
		// HDR require meta data information in video frame
		// note 'transfer' can have two possible values for HDR: bt_2100_pq or bt_2100_hlg
		frame.p_metadata =
			"<ndi_color_info "
...
int main(int argc, char* argv[])
{
	...
	NDI_send_description.p_ndi_name = "HEVC HDR Test";
