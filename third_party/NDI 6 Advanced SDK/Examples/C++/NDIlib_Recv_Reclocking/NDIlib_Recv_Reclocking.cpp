...
int main(int argc, char* argv[])
{
	...
	// Create a high quality frame-sync
	NDIlib_framesync_instance_t pNDI_framesync_highQ = NDIlib_framesync_create(pNDI_recv_highQ);
	if (!pNDI_framesync_highQ)
		return 0;
	...
	// Create a high quality frame-sync
	NDIlib_framesync_instance_t pNDI_framesync_lowQ = NDIlib_framesync_create(pNDI_recv_lowQ);
	if (!pNDI_framesync_lowQ)
		return 0;

	// Create an NDI source that is called "My Video and Audio" and is clocked to the video.
	NDIlib_send_create_t NDI_send_create_desc;
	NDI_send_create_desc.clock_video = NDI_send_create_desc.clock_audio = true;
	...
	auto forward_video_highQ_fcn = [=](bool& exit) {
		while (!exit) {
			// Get the video frame from a frame-sync then resend it
			NDIlib_video_frame_v2_t video_frame;
			NDIlib_framesync_capture_video(pNDI_framesync_highQ, &video_frame);
			NDIlib_send_send_video_v2(pNDI_send, &video_frame);
			NDIlib_framesync_free_video(pNDI_framesync_highQ, &video_frame);
		}
	};
	...
	auto forward_video_lowQ_fcn = [=](bool& exit) {
		while (!exit) {
			// Get the video frame
			NDIlib_video_frame_v2_t video_frame;
			NDIlib_framesync_capture_video(pNDI_framesync_lowQ, &video_frame);
			NDIlib_send_send_video_v2(pNDI_send, &video_frame);
			NDIlib_framesync_free_video(pNDI_framesync_lowQ, &video_frame);
		}
	};
	...
	auto forward_audio_fcn = [=](bool& exit) {
		while (!exit) {
			...
			NDIlib_framesync_capture_audio_v2(pNDI_framesync_highQ, &audio_frame, 48000, 2, 600);
			NDIlib_send_send_audio_v3(pNDI_send, &audio_frame);
			NDIlib_framesync_free_audio_v2(pNDI_framesync_highQ, &audio_frame);
		}
	};
