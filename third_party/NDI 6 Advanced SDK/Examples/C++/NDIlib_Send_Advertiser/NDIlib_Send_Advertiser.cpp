...
int main(int argc, char* argv[])
{
	...
	NDIlib_send_advertiser_instance_t pNDI_sender_advertiser = NDIlib_send_advertiser_create();
	if (!pNDI_sender_advertiser) {
		printf("The sender advertiser failed to create. Please configure the connection to the NDI discovery server.\n");
		NDIlib_send_destroy(pNDI_send);
		NDIlib_destroy();
		return 0;
	}

	// Add the sender to the advertiser for monitoring purposes.
	NDIlib_send_advertiser_add_sender(pNDI_sender_advertiser, pNDI_send, true);
	...
	// Remove the sender from the advertiser.
	NDIlib_send_advertiser_del_sender(pNDI_sender_advertiser, pNDI_send);

	// Destroy the sender advertiser.
	NDIlib_send_advertiser_destroy(pNDI_sender_advertiser);
