...
{
#ifdef _WIN32
	// We check whether the NDI runtime is installed.
	...
	if (!p_ndi_runtime_v6) {
		...
		MessageBoxA(NULL, "Please install the NDI runtime to use this application.", "Runtime Warning.", MB_OK);
	...
	if (hNDILib)
		*((FARPROC*)&NDIlib_v6_load) = GetProcAddress(hNDILib, "NDIlib_v6_load");
	...
	if (!NDIlib_v6_load) {
		...
		// The NDI runtime is not installed correctly. Let the user know and take them to the download URL.
		MessageBoxA(NULL, "Please re-install the NDI runtime to use this application.", "Runtime Warning.", MB_OK);
		ShellExecuteA(NULL, "open", NDILIB_REDIST_URL, 0, 0, SW_SHOWNORMAL);
		return 0;
	}
#else
	...
	if (!NDIlib_v6_load) {
		...
		printf("Please re-install the NewTek NDI runtime from " NDILIB_REDIST_URL " to use this application.");
		return 0;
	}
#endif
	...
	// We are going to create a 1920x1080 interlaced frame at 29.97 Hz.
